...
$sql = "SELECT f.feedback_id, c.c_fullname, f.rating, f.comment, f.date_created
        FROM feedback f
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['search'])) {
    ...
    $sql = "SELECT f.feedback_id, c.c_fullname, f.rating, f.comment, f.date_created
            FROM feedback f
            INNER JOIN client c ON f.client_id = c.client_id
            WHERE c.c_fullname LIKE '%$search%'
            ORDER BY f.date_created DESC";
}
...
<div class="header">
    ...
    <p class="heading-main12" style="font-size: 24px; font-weight: bold; text-align: center; margin-bottom: 20px;">CLIENT FEEDBACK DETAILS</p>
...
<?php
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['search'])) {
    echo "<p>Search results for '<strong>" . htmlspecialchars($_POST['search']) . "</strong>':</p>";
    if ($result->num_rows == 0) {
        ...
        <table class="sub-table" style="width: 100%; border-collapse: collapse; text-align: center;">
            ...
            <thead>
                <tr>
                    ...
                    <th style="font-size: 16px; font-weight: bold; padding: 10px; border-bottom: 2px solid #ddd; text-align: center;">Rating</th>
                    <th style="font-size: 16px; font-weight: bold; padding: 10px; border-bottom: 2px solid #ddd; text-align: center;">Comment</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td style='padding: 10px; border-bottom: 1px solid #ddd;'>{$row['c_fullname']}</td>
                        <td style='padding: 10px; border-bottom: 1px solid #ddd;'>{$row['rating']}</td>
                        <td style='padding: 10px; border-bottom: 1px solid #ddd;'>{$row['comment']}</td>
                        <td style='padding: 10px; border-bottom: 1px solid #ddd;'>{$row['date_created']}</td>
                      </tr>";
                $count++;
            }
        } else {
            echo "<tr><td colspan='5' style='padding: 10px; border-bottom: 1px solid #ddd;'>No feedback available</td></tr>";
        }
        ?>
    </tbody>
        </table>
